  ...
  <tr>
    <td align="center">
      <a href="https://github.com/jeongjeson656">
        <img src="https://github.com/jeongjeson656.png" width="100px;" alt="손정제" />
        <br />
        <b>손정제</b>
      </a>
      <br />
...
<p>
  ...
  <img src="https://img.shields.io/badge/Android%20Phone-3DDC84?style=for-the-badge&logo=android&logoColor=white" alt="Android Phone" />
  <img src="https://img.shields.io/badge/DHT11-0F766E?style=for-the-badge" alt="DHT11" />
  <img src="https://img.shields.io/badge/Ultrasonic%20Sensor-2563EB?style=for-the-badge" alt="Ultrasonic Sensor" />
  <img src="https://img.shields.io/badge/I2C%20LCD-111827?style=for-the-badge" alt="I2C LCD" />
  <img src="https://img.shields.io/badge/DFPlayer%20Mini-B91C1C?style=for-the-badge" alt="DFPlayer Mini" />
  <img src="https://img.shields.io/badge/Servo%20Motor-7C3AED?style=for-the-badge" alt="Servo Motor" />
  <img src="https://img.shields.io/badge/Stepper%20Motor-374151?style=for-the-badge" alt="Stepper Motor" />
</p>
...
<p>
  ...
  <img src="https://img.shields.io/badge/Android%20BLE-3DDC84?style=for-the-badge&logo=android&logoColor=white" alt="Android BLE" />
  ...
  <img src="https://img.shields.io/badge/CRC--8-7C3AED?style=for-the-badge" alt="CRC-8" />
</p>
...
서보모터 기반 전동문과 STM32 기반 전동경사로 제어부를 연동합니다.
...
<table>
  <tr>
    <td>
      ...
      LCD/오디오 피드백</strong>입니다.
      <br /><br />
      여기에 초음파 센서 기반 끼임방지, DHT11 기반 화재감지, 하차벨, 실시간
      LCD 모니터링, 음성 안내를 함께 구성하여 버스 출입 장치의 편의성과
      안전성을 통합적으로 다룹니다.
    </td>
  </tr>
</table>
...
- 전동문, 전동경사로, 끼임감지, 하차벨 상태를 LCD에 실시간 표시하는 모니터링 기능 구현
...
| **운전석 ECU 제어 로직** | 하차벨, 장애인 하차벨, 전동문 버튼, 전동경사로 버튼, BLE UART 명령, 화재감지 상태를 통합 판단합니다.<br />Door ECU로 CAN command를 송신하고 Door/Slope/Block/Stop 상태를 LCD와 오디오 안내로 피드백합니다. |
...
| **UART 신뢰성 프레임** | BLE UART와 Slope UART는 6바이트 고정 프레임을 사용합니다.<br />`0xA5 0x5A` SOF, DATA/ACK 타입, `seq`, `cmd`, CRC-8 검증, ACK timeout, retry limit, 중복 프레임 억제 구조를 갖습니다. |
| **전동문 및 전동경사로 구동** | 전동문은 TC375 CCU6 PWM 기반 서보모터로 duty를 점진적으로 변경하며 열림/닫힘을 수행합니다.<br />전동경사로는 STM32 HAL 기반 스텝모터 시퀀스를 통해 전개/수납 동작을 수행합니다. |
| **안전 및 안내 기능** | 초음파 센서는 약 30cm 이내 장애물을 감지해 끼임방지 상태를 생성합니다.<br />DHT11 온도 기반 화재감지 시 화재 안내 음성을 재생하고 전동문 개방 명령을 발생시킵니다. DFPlayer Mini는 문 열림/닫힘, 경사로 전개/수납, 화재 경보 음원을 재생합니다. |
| **실시간 모니터링** | I2C LCD에 Door, Slope, Block, Stop 상태를 비트마스킹 기반으로 표시합니다.<br />운전자는 전동문, 전동경사로, 끼임감지, 하차벨 상태를 즉시 확인할 수 있습니다. |
...
끼임감지 상태를 업데이트하고 LCD 및 오디오 모듈에 반영합니다.

전동문 ECU는 CAN command를 받아 전동문 서보모터를 제어하고, 경사로 요청은 Slope UART를
...
flowchart LR
    ...
    Slope --> Ramp["Stepper Motor<br/>Electric Ramp"]
    Door --> DoorMotor["Servo Motor<br/>Electric Door"]
    Obstacle["Ultrasonic Sensor<br/>Pinch Detection"] --> Door
    Fire["DHT11<br/>Fire Detection"] --> Driver
    StopBell["Stop Bell Buttons"] --> Driver
    Driver --> Audio["DFPlayer Mini<br/>Speaker"]
    Driver --> Monitor["I2C LCD<br/>Door/Slope/Block/Stop"]
```
...
또한 1바이트 CAN payload 설계, CRC-8 기반 UART DATA/ACK 프레임, polling 중심의
...
| 구분 | 내용 |
| --- | --- |
| **프로젝트 개요서** | 프로젝트 목적, 팀 구성, 일정, 기술스택, 핵심 기능 정리 |
| **최종 결과보고서** | 발표자료 형식의 프로젝트 개요, 기능 소개, ECU 구성, 시연 및 회고 |
| **요구사항 & 테스트케이스 문서** | 요구사항 정의, 통합 테스트 설계서, 시나리오/인수 테스트 설계서 |
| **인터페이스 & 모듈 설계서** | CAN 라이브러리, Driver ECU-Door ECU CAN, BLE/Slope UART, 전동문 ECU, 운전석 ECU, 오디오, 모니터링, 하차벨, 화재감지, 핀맵 문서 |
